...
const ViewNilai5 = () => {
   ...
   useEffect(() => {
    //student
    firebase.database().ref(`/${angkatan}/${id}/kelas3semganjil/data`).on("value", (res) => {
      if(res.val()) {
          //ubah menjadi array
  ...
  const onSubmit = () =>{
    ...
    firebase.database().ref(`/${angkatan}/${id}/kelas1semganjil/data/${selectedKegiatan.id}`).update(data);
    resetform();
}
const resetform = () =>{
    setNilai('')
