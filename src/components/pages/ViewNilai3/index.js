...
const ViewNilai3 = () => {
   ...
   useEffect(() => {
    //student
    firebase.database().ref(`/${angkatan}/${id}/kelas2semganjil/data`).on("value", (res) => {
      if(res.val()) {
          //ubah menjadi array
  ...
  const onSubmit = () =>{
    ...
    firebase.database().ref(`/${angkatan}/${id}/kelas2semganjil/data/${selectedKegiatan.id}`).update(data);
    resetform();
}
const resetform = () =>{
    setNilai('')
